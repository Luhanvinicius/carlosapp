// app/app/admin/quadras/page.tsx - Quadras admin (igual ao cursor)
'use client';

import { useEffect, useState } from 'react';
import { pointService, quadraService } from '@/services/agendamentoService';
import type { Point, Quadra, CriarQuadraPayload } from '@/types/agendamento';
...
export default function AdminQuadrasPage() {
  ...
  const [pointFiltro, setPointFiltro] = useState<string>('');
  ...
  const carregarDados = async () => {
    try {
      setLoading(true);
      const [pointsData, quadrasData] = await Promise.all([
        pointService.listar(),
  ...
  const carregarQuadras = async () => {
    try {
      const data = await quadraService.listar(pointFiltro || undefined);
      setQuadras(data);
    } catch (error) {
  ...
  const quadrasFiltradas = pointFiltro
    ? quadras.filter((q) => q.pointId === pointFiltro)
    : quadras;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Gerenciar Quadras</h1>
          <p className="text-gray-600">Administre as quadras dos estabelecimentos</p>
        </div>
        <button
          onClick={() => abrirModal()}
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="mb-4">
          ...
          <select
            value={pointFiltro}
            onChange={(e) => setPointFiltro(e.target.value)}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left py-3 px-4 font-semibold text-gray-700">Estabelecimento</th>
              ...
              <tbody>
                {quadrasFiltradas.map((quadra) => {
                  const point = points.find((p) => p.id === quadra.pointId);
                  return (
                    <tr key={quadra.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4 text-gray-600">{point?.nome || '—'}</td>
